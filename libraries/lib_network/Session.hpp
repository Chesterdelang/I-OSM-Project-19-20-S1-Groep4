...
#ifndef LIBRARIES_LIB_NETWORK_SESSION_HPP_
#define LIBRARIES_LIB_NETWORK_SESSION_HPP_
...
protected:
	...
	void readMessage() {
		...
		boost::asio::async_read(getSocket(), boost::asio::buffer(headerBuffer),
				boost::bind(&Session::handleHeaderRead, this, aMessage,
	...
	void handleBodyRead(Message &aMessage,
		...
		if (!error) {
			...
			handleMessageRead(aMessage, error);
		} else {
			// See https://isocpp.org/wiki/faq/freestore-mgmt#delete-this
			delete this;
	...
	void writeMessage(Message &aMessage) {
		boost::asio::async_write(getSocket(),
				boost::asio::buffer(aMessage.getHeader().toString(),
						aMessage.getHeader().getHeaderLength()),
				boost::bind(&Session::handleHeaderWritten, this, aMessage,
						boost::asio::placeholders::error));
	...
	void handleHeaderWritten(Message &aMessage,
		...
		if (!error) {
			boost::asio::async_write(getSocket(),
					...
					boost::bind(&Session::handleBodyWritten, this, aMessage,
							boost::asio::placeholders::error));
		} else {
			delete this;
			throw std::runtime_error(
...
#endif /* LIBRARIES_LIB_NETWORK_SESSION_HPP_ */
